...
function EditHabit({ setLoading }) {
	...
	const updateHabit = async () => {
		...
		try {
			...
			prevUserHabits.splice(prevUserHabits.indexOf(states.habitToBeEdited),1, habit);
			// console.log(prevUserHabits.indexOf(states.habitToBeEdited));
			setStates((prev)=>({...prev,openEditHabitModal:false, userHabits:prevUserHabits}));
		} catch (error) {
			console.error(error);
		}
		setLoading(false);
		...
		<div className={Styles.add_habit}>
			<div className={Styles.add_habit_wrapper}>
				<div className={Styles.add_habit_main_wrapper}>
					...
					<p className="days_selected_err" style={{
				...
				<div className={Styles.bottom_div}>
					<Button className={Styles.add_btn} onClick={() => {
							if(habit.daysSelected.length==0){
								document.querySelector(".days_selected_err").innerHTML = "Select Some Days First!"
							}else{
								updateHabit();
							}
						}}>
