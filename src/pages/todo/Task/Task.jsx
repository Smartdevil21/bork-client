	...
	async function deleteTask(taskDataArg) {
		try {
			...
			setStates((prev) => ({
				...
				completedTask: completedTask.filter((task, index) => task !== taskDataArg),
			}));
		} catch (error) {
			console.log(error);
		}
	...
	return (
		<div className={Styles.task} style={{ opacity: completed ? 0.5 : 1 }}>
			...
			>
				...
				<div className={Styles.priority} style={{backgroundColor:taskData.task.priority==0?"#FF6363":taskData.task.priority==1?"#FFC700":"#6EEF9A"}}></div>
			</div>
