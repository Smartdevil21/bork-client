...
function Home() {
	...
	//TO GET THE USER ID THROUGH URL
	...
	const getTasksOfUser = async () => {
		...
		try {
			...
			const userTasks = tasksDetails.data;
			setStates((prev) => ({
				...prev,
	...
	const getUserHabits = async ()=>{
		try {
			...
			setStates((prev)=>({...prev, userHabits:result.data}));
		} catch (error) {
			console.error(error);
		}
	}
					...
					<div className={Styles.main_wrapper}>
						<div className={Styles.horizontal_divs}>
							<div className={Styles.time_div}>
								<img src={todayInfo.getHours()<12?morning:todayInfo.getHours()>17?evening:afternoon} alt="" />
								<div className={Styles.time_div_text}>
									<p className={Styles.thought}>{qoute}</p>
									<div className={Styles.time_day}>
										<div className={Styles.time}>
											<h1>{`${todayInfo.getHours()}:${todayInfo.getMinutes()<10?"0"+todayInfo.getMinutes():todayInfo.getMinutes()}`}</h1>
							...
							<div className={Styles.remaining_tasks}>
								...
								<div className={Styles.task}>
									<Stack direction={"column"} spacing={1}>
										{states.userTasks
											.filter((task, index) => task.task.status === "progress")
											.map((userTask, index) => {
						...
						<div className={Styles.user_profile}>
							<div className={Styles.user_image}>
								{/* <img src={morning} alt="" /> */}
								<Icon icon="bi:person-fill" color="gray" width="32" height="32" />
							</div>
							<div className={Styles.user_greetings_text}>
								<p>{todayInfo.getHours()<12?"Good Morning":todayInfo.getHours()>17?"Good Evening":"Good Afternoon"},</p>
